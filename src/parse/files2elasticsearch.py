...
class FileToElasticSearch:
    def __init__(self,
        ...
        :param es_host: Host for Elasticsearch, default is "localhost".
        :param es_port: Port for Elasticsearch, default is 9200.
        :param index_name: Index name in Elasticsearch, default is "documents".
        :param embedding_model: Embedding model to use, default is "qwen:7b".
        """
        self.index_name = index_name
        self.es_client = Elasticsearch([{"host": es_host, "port": es_port, "scheme": "http"}]) # TEST SETTING: HTTP, NO CONFIGURATION
    ...
    def process_file(self, file_path: str):
        """Process a single file and index the chunks into Elasticsearch."""
        ...
        # Embed and prepare documents for Elasticsearch
        print(f"Indexing chunks into Elasticsearch for file: {file_path}")
        actions = []
        for i, chunk in enumerate(all_splits):
            # Generate embedding
            embedding = self.embeddings.embed_query(chunk)
            # Prepare document for bulk indexing
            doc_id = f"{file_hash}-{i}"
            actions.append({
        ...
        print(f"Successfully indexed {success} chunks into Elasticsearch.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process files and index them into Elasticsearch.")
    ...
    parser.add_argument("--index_name", type=str, default="documents", help="Elasticsearch index name.")
    parser.add_argument("--embedding_model", type=str, default="qwen:7b",
                        help="Embedding model to use for generating vectors.")
    args = parser.parse_args()

    # Initialize FileToElasticSearch class
    file_to_es = FileToElasticSearch(es_host=args.es_host,
    ...
    if args.input:
        if os.path.isfile(args.input):
            file_to_es.process_file(args.input)
        elif os.path.isdir(args.input):
            file_to_es.process_directory(args.input)
        else:
        ...
        file_to_es.process_directory(example_dir)